...
async function main() {
    ...
    for (const sql of FOREIGN_KEYS) {
        const existing = await prisma.$queryRawUnsafe(`SELECT 1 FROM pg_constraint WHERE conname = $1`, sql.match(/CONSTRAINT "([^"]+)"/)?.[1]);
...
//# sourceMappingURL=apply-guard-compliance-table.js.map