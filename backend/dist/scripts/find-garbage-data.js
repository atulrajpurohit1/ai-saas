...
const KNOWN_GARBAGE_PATTERN = /\b(test|mock|xyz|john\s+doe|jane\s+smith|lorem\s+ipsum|asdf|qwerty|foo\s*bar|sample\s+data|ghost)\b/i;
function vowelRatio(cleaned) {
    if (cleaned.length === 0)
        return 1;
...
function checkValue(value) {
    if (!value || !value.trim())
        return null;
    if (KNOWN_GARBAGE_PATTERN.test(value))
        return 'known-garbage-term';
...
async function main() {
    const findings = [];
    const record = (table, id, tenantId, fields) => {
        for (const [field, value] of Object.entries(fields)) {
            const reason = checkValue(value);
            if (reason)
                findings.push({ table, id, tenantId, field, value: value, reason });
        }
    };
    const [leads, deals, sites, branches, guards, incidents, checkpoints, clients, tenants] = await Promise.all([
    ...
    incidents.forEach((r) => record('Incident', r.id, r.tenantId, { title: r.title, description: r.description }));
    checkpoints.forEach((r) => record('Checkpoint', r.id, r.tenantId, { name: r.name, locationNote: r.locationNote }));
    clients.forEach((r) => record('Client', r.id, r.tenantId, { name: r.name, companyName: r.companyName }));
...
//# sourceMappingURL=find-garbage-data.js.map