...
let SalesAcceleratorService = SalesAcceleratorService_1 = class SalesAcceleratorService {
    ...
    async getForecastReport(tenantId) {
        const dashboard = await this.getDashboard(tenantId);
        const forecastDeals = [
        ...
        const uniqueDeals = Array.from(new Map(forecastDeals.map((deal) => [deal.id, deal])).values());
        const weightedPipeline = uniqueDeals.reduce((sum, deal) => sum + deal.forecast.probability, 0);
        return {
            generatedAt: dashboard.generatedAt,
            summary: {
    ...
    async getCoachingAnalytics(tenantId) {
        const dashboard = await this.getDashboard(tenantId);
        ...
        const reps = repIds.map((repId) => {
            ...
            const objectionCount = repAssessments.reduce((sum, item) => sum + item.objectionRisks.length, 0) +
                repDiscoveries.reduce((sum, item) => sum + item.objections.length, 0);
            const riskCoverage = this.average(repDiscoveries.map((item) => (item.riskConcerns.length > 0 ? 100 : 0)));
            const painCoverage = this.average(repDiscoveries.map((item) => (item.painPoints.length > 0 ? 100 : 0)));
            const user = userMap.get(repId);
            const coachingScore = this.clamp((avgDiscovery ?? 50) * 0.35 +
            ...
            return {
                ...
                coachingScore,
                recommendedAction: avgDiscovery !== null && avgDiscovery < 60
    ...
    async getLearningLoop(tenantId) {
        const dashboard = await this.getDashboard(tenantId);
        const learnings = dashboard.postCloseFeedbackDeals.map((deal) => ({
            ...
            recommendedAction: deal.postCloseFeedback.recommendedAction,
            proposalWarning: deal.postCloseFeedback.status === 'oversold'
    ...
    async getDealWorkspace(tenantId, dealId) {
        ...
        const rateCardPricing = await this.rateCardPricingInsight(tenantId, deal, discovery, valueJustification);
        ...
        const followUpSequenceProgress = this.followUpSequenceProgress(deal.activities || []);
        return {
            deal,
            discovery,
            assessment,
    ...
    async rateCardPricingInsight(tenantId, deal, discovery, valueJustification) {
        const now = new Date();
        const baseWhere = {
            tenantId,
            status: 'active',
        ...
        const estimatedMonthlyGuardHours = valueJustification?.estimatedMonthlyGuardHours ??
            this.estimatedMonthlyGuardHours(discovery?.guardCount, discovery?.serviceHours);
        const estimatedMonthlyRevenue = estimatedMonthlyGuardHours !== null && averageHourlyRate !== null
            ? this.roundCurrency(estimatedMonthlyGuardHours * averageHourlyRate)
            : null;
        const lowMonthlyRevenue = estimatedMonthlyGuardHours !== null && minHourlyRate !== null
            ? this.roundCurrency(estimatedMonthlyGuardHours * minHourlyRate)
            : null;
        const highMonthlyRevenue = estimatedMonthlyGuardHours !== null && maxHourlyRate !== null
        ...
        const status = benchmarkSource === 'none'
            ? 'missing_rate_card'
    ...
    rateCardPricingContext(pricing) {
        return [
    ...
    proposalEngagement(proposals) {
        const sorted = [...proposals].sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime());
        const latest = sorted[0] ?? null;
        const totalCommentCount = sorted.reduce((sum, proposal) => sum + (proposal._count?.comments ?? 0), 0);
        if (!latest) {
            return {
                status: 'not_started',
                score: 20,
        ...
        const status = this.proposalEngagementStatus(latestStatus, engagementScore, ageDays, latest._count?.comments ?? 0);
        return {
            status,
    ...
    proposalEngagementStatus(latestStatus, score, ageDays, commentCount) {
        if (/approved|accepted|won/.test(latestStatus))
            return 'approved';
        if (/reject|declin|lost/.test(latestStatus))
            return 'rejected';
        if (/draft/.test(latestStatus))
            return 'draft';
        if (ageDays > 14)
            return 'stale';
        if (commentCount > 0 || score >= 70)
            return 'engaged';
        if (/sent|shared|pending|review/.test(latestStatus))
            return 'sent';
        return score < 45 ? 'stale' : 'sent';
    }
    proposalEngagementAction(status) {
        if (status === 'approved') {
    ...
    followUpSequenceProgress(activities) {
        const sequenceActivities = (activities || [])
            .filter((activity) => activity.subject.toLowerCase().includes('[sales sequence]'))
            .sort((a, b) => {
        ...
        const now = new Date();
        const completedActivities = sequenceActivities.filter((activity) => /(completed|done|closed)/i.test(activity.status));
        const pendingActivities = sequenceActivities.filter((activity) => !/(completed|done|closed)/i.test(activity.status));
        const overdueActivities = pendingActivities.filter((activity) => activity.dueDate && activity.dueDate.getTime() < now.getTime());
        const nextStep = pendingActivities.find((activity) => !activity.dueDate || activity.dueDate.getTime() >= now.getTime()) ||
            overdueActivities[0] ||
            null;
        const lastCompletedStep = [...completedActivities].sort((a, b) => (b.dueDate || b.createdAt).getTime() -
            (a.dueDate || a.createdAt).getTime())[0] || null;
        const latestSequenceActivity = [...sequenceActivities].sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime())[0] || null;
        const daysSinceLastSequenceActivity = latestSequenceActivity
            ? this.daysBetween(latestSequenceActivity.createdAt, now)
            : null;
        const completionRate = Math.round((completedActivities.length / sequenceActivities.length) * 100);
        const risks = [];
        const signals = [];
        ...
        if (daysSinceLastSequenceActivity !== null &&
            daysSinceLastSequenceActivity > 10 &&
            pendingActivities.length > 0) {
            risks.push('Sequence has pending work and no new sequence activity in over 10 days.');
        }
        const status = this.followUpSequenceProgressStatus(completionRate, overdueActivities.length, pendingActivities.length, daysSinceLastSequenceActivity);
        return {
            status,
    ...
    followUpSequenceProgressStatus(completionRate, overdueCount, pendingCount, daysSinceLastSequenceActivity) {
        if (completionRate === 100)
            return 'completed';
        if (overdueCount > 0)
            return 'overdue';
    ...
    followUpSequenceProgressAction(status, nextStep) {
        if (status === 'completed') {
...
Confirm final coverage hours, decision timeline, and approval stakeholders, then finalize pricing and launch plan.
    ...
    averageCurrency(values) {
        if (values.length === 0)
            return null;
        return this.roundCurrency(values.reduce((sum, value) => sum + value, 0) / values.length);
    }
    roundCurrency(value) {
