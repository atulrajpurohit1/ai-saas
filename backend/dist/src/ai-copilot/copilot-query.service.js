...
exports.CopilotQueryService = void 0;
...
let CopilotQueryService = class CopilotQueryService {
    prisma;
    aiInsightsService;
    revenueInsightsService;
    recommendationService;
    constructor(prisma, aiInsightsService, revenueInsightsService, recommendationService) {
        this.prisma = prisma;
        this.aiInsightsService = aiInsightsService;
        this.revenueInsightsService = revenueInsightsService;
        this.recommendationService = recommendationService;
    }
    async answerQuestion(tenantId, userId, question) {
        ...
        if (intent === 'incidents' || intent === 'sites') {
            return this.answerIncidentOrSiteQuestion(tenantId, userId, normalized, question);
        }
        if (intent === 'reports') {
    ...
    detectIntent(question) {
        if (/(invoice|billing|bill|unpaid|overdue|outstanding|paid|payment|dispute|collection)/.test(question)) {
            return 'billing';
        }
        if (/(revenue|forecast|renewal|contract|growth|highest revenue|client value)/.test(question)) {
            return 'revenue';
        }
        if (/(missed|attendance|late|staff|staffing|shortage|coverage|shift|guard)/.test(question)) {
            return /(guard)/.test(question) ? 'guards' : 'staffing';
        }
        if (/(incident|risk|threat|unauthorized|access|case|resolution)/.test(question)) {
            return 'incidents';
        }
    ...
    async answerIncidentOrSiteQuestion(tenantId, userId, normalized, question) {
        if (/(risk|risky|top risk|critical)/.test(normalized)) {
            const incidentInsights = await this.aiInsightsService.getIncidentInsights(tenantId, userId);
            ...
            return {
                ...
                sources: rows.map((row) => this.source('site', row.entityId, row.name, `/sites/${row.entityId}`, row.indicators.join('; '))),
                actions: rows.map((row) => this.action('View Site', 'site', `/sites/${row.entityId}`)),
        ...
        return {
            ...
            sources: rows.map((row) => this.source('site', row.siteId, row.name, `/sites/${row.siteId}`, `${row.incidentCount} incidents, ${row.incidentRate}% incident rate`)),
            actions: rows.map((row) => this.action('View Site', 'site', `/sites/${row.siteId}`)),
            knowledgeQuery: `${question} ${rows.map((row) => `${row.name} incidents ${row.incidentCount}`).join(' ')}`,
            context: { sites: rows },
        };
    }
    async answerBillingQuestion(tenantId, question) {
        const now = new Date();
    ...
    async answerStaffingQuestion(tenantId, normalized, question) {
        if (/(shortage|coverage|unassigned|staffing)/.test(normalized)) {
            const overview = await this.recommendationService.getSchedulingOverview(tenantId);
            const gaps = overview.gaps.slice(0, 5);
            const answer = overview.shortageSlots > 0
            ...
            return {
                ...
                sources: gaps.map((gap) => this.source('shift', gap.shiftId, gap.siteName, `/shifts/${gap.shiftId}`, `${gap.shortageSlots} open slot${gap.shortageSlots === 1 ? '' : 's'}`)),
                actions: gaps.map((gap) => this.action('View Site', 'site', `/sites/${gap.siteId}`)),
    ...
    async answerRevenueQuestion(tenantId, userId, normalized, question) {
        if (/(forecast|growth|next month|quarter|annual)/.test(normalized)) {
            const revenue = await this.revenueInsightsService.getRevenueDashboard(tenantId, userId);
        ...
        if (/(renewal|contract)/.test(normalized)) {
            const renewals = await this.revenueInsightsService.getRenewalOpportunities(tenantId, userId);
            ...
            return {
                ...
                sources: rows.map((row) => this.source('client', row.clientId, row.name, `/clients/${row.clientId}`, row.reason)),
                actions: rows.map((row) => this.action('View Client', 'client', `/clients/${row.clientId}`)),
                knowledgeQuery: `${question} renewal contract ${rows.map((row) => `${row.name} ${row.reason}`).join(' ')}`,
                context: { renewals: rows },
            };
        }
        const clientValue = await this.revenueInsightsService.getClientValueAnalysis(tenantId, userId);
        ...
        return {
            ...
            sources: rows.map((row) => this.source('client', row.clientId, row.name, `/clients/${row.clientId}`, `${this.formatCurrency(row.totalRevenue)} revenue, ${row.revenueShare}% share`)),
            actions: rows.map((row) => this.action('View Client', 'client', `/clients/${row.clientId}`)),
            knowledgeQuery: `${question} highest revenue clients ${rows.map((row) => row.name).join(' ')}`,
            context: { clients: rows },
        };
    }
    async answerReportsQuestion(tenantId, question) {
        ...
        return {
            ...
            sources: reports.map((report) => this.source('report', report.id, `${report.site.name} report`, `/reports/${report.id}`, `${report.status} - ${report.reportDate.toISOString().slice(0, 10)}`)),
            actions: reports.map((report) => this.action('View Report', 'report', `/reports/${report.id}`)),
            knowledgeQuery: `${question} reports daily service ${reports.map((report) => report.site.name).join(' ')}`,
            context: { reports },
        };
    }
    async answerGeneralQuestion(tenantId, userId, question) {
        ...
        const topRecommendation = dashboard.recommendations[0]?.action ||
    ...
    async getMissedShiftsLastWeek(tenantId) {
        ...
        shifts.forEach((shift) => {
            const checkedIn = new Set(shift.attendanceEvents.map((event) => event.guardId));
            shift.assignments.forEach((assignment) => {
                if (checkedIn.has(assignment.guardId))
                    return;
...
exports.CopilotQueryService = CopilotQueryService;
exports.CopilotQueryService = CopilotQueryService = __decorate([
...
//# sourceMappingURL=copilot-query.service.js.map