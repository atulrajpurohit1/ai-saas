...
type HistoricalShift = {
  ...
  requiredGuards: number;
  status: string;
  assignments: Array<{ guardId: string }>;
  attendanceEvents: Array<{ guardId: string; type: string; timestamp: Date }>;
};
...
type InvoiceRecord = {
  ...
  disputes: Array<{ status: string; createdAt: Date; resolvedAt: Date | null }>;
};
...
type PredictionContext = {
  ...
  now: Date;
  horizonEnd: Date;
  staffingHistoryStart: Date;
  currentIncidentStart: Date;
  previousIncidentStart: Date;
...
export class PredictionEngineService {
  ...
  async getDashboard(tenantId: string, userId: string): Promise<PredictionDashboard> {
    ...
    const attachedRecommendations = this.aiMonitoringService.attachGenerationId(
  ...
  private async loadContext(tenantId: string, userId: string): Promise<PredictionContext> {
    ...
    return {
      ...
      upcomingShifts: upcomingShifts as UpcomingShift[],
      historicalShifts: historicalShifts as HistoricalShift[],
      availabilities: availabilities as AvailabilityRecord[],
      incidents: incidents as IncidentRecord[],
      invoices: invoices as InvoiceRecord[],
      rateCards: rateCards as RateCardRecord[],
      opsDashboard,
      incidentInsights,
      revenueDashboard,
      schedulingOverview,
  ...
  private buildSiteStaffingPrediction(
    ...
    const conflictCount = upcoming.reduce(
      (sum, shift) => sum + this.shiftConflictCount(context, shift),
      0,
    );
    ...
    if (score < 25 && upcoming.length > 0) return null;
    if (upcoming.length === 0 && score < 40) return null;

    const confidenceScore = this.confidenceScore(
      historical.length + upcoming.length,
      [shortageSlots, conflictCount, historicalCoverageIssues, assignedPast],
    );
  ...
  private buildWeekendNightStaffingPrediction(context: PredictionContext): StaffingPrediction | null {
    ...
    const otherHistory = context.historicalShifts.filter(
      (shift) => !this.isWeekendNight(shift.startTime),
    );
    ...
    const upcomingWeekendNight = context.upcomingShifts.filter((shift) =>
      this.isWeekendNight(shift.startTime),
    );
    const riskIncrease =
      otherIssueRate > 0
    ...
    return {
      ...
      conflictCount: upcomingWeekendNight.reduce(
        (sum, shift) => sum + this.shiftConflictCount(context, shift),
        0,
      ),
    };
  ...
  private buildIncidentPredictions(context: PredictionContext): PredictionSection<IncidentPrediction> {
    ...
    const previousIncidents = context.incidents.filter(
      (incident) =>
  ...
  private buildIncidentTypeTrendPrediction(context: PredictionContext): IncidentPrediction | null {
    ...
    const currentByType = this.countBy(currentIncidents, (incident) =>
      this.incidentType(incident.title, incident.description),
    );
    const previousByType = this.countBy(previousIncidents, (incident) =>
      this.incidentType(incident.title, incident.description),
    );
  ...
  private buildPaymentRiskPredictions(context: PredictionContext): PredictionSection<PaymentRiskPrediction> {
    ...
    const predictions = Array.from(invoicesByClient.entries())
      .map(([clientId, invoices]) => {
        ...
        } satisfies PaymentRiskPrediction;
      })
      .filter((prediction) => prediction.riskScore >= 25)
      .sort((left, right) => right.riskScore - left.riskScore)
      .slice(0, 8);
  ...
  private buildRenewalRiskPredictions(context: PredictionContext): PredictionSection<RenewalRiskPrediction> {
    ...
    const predictions = context.revenueDashboard.contracts.rows
      .map((contract) => {
        ...
        const nonRenewalProbability = this.roundRiskScore(
          (100 - contract.healthScore) * 0.45 +
            urgency +
        ...
        return {
          ...
          contractHealthTrend,
        } satisfies RenewalRiskPrediction;
      })
      .filter(
        (prediction) =>
  ...
  private buildRecommendations(input: {
    ...
    payments: PaymentRiskPrediction[];
    renewals: RenewalRiskPrediction[];
  }): AiRecommendation[] {
  ...
  private toRecommendation(
    ...
    input: {
      ...
      title: string;
      action: string;
      actionType: NonNullable<AiRecommendation['actionType']>;
      targetModule: NonNullable<AiRecommendation['targetModule']>;
    },
  ): AiRecommendation {
    return {
      id: input.id,
  ...
  private shiftConflictCount(context: PredictionContext, shift: UpcomingShift) {
    return shift.assignments.filter((assignment) => {
      const unavailable = context.availabilities.some(
        (availability) =>
          availability.guardId === assignment.guardId &&
          this.datesOverlap(
            availability.startDate ?? new Date(0),
  ...
  private countAvailabilityConflicts(context: PredictionContext) {
    return context.upcomingShifts.reduce(
      (sum, shift) => sum + this.shiftConflictCount(context, shift),
      0,
    );
  }
  ...
  private emptyChurnPrediction(context: PredictionContext): ChurnPrediction {
    return {
      id: 'churn-empty',
      category: 'churn',
      entityType: 'client',
  ...
  private daysPastDue(invoice: InvoiceRecord, now: Date) {
  ...
  private isWeekendNight(date: Date) {
    const day = date.getUTCDay();
    const hour = date.getUTCHours();
    return (day === 0 || day === 5 || day === 6) && (hour >= 18 || hour < 6);
  }

  private riskLevel(score: number): PredictionRiskLevel {
    if (score >= 70) return 'high';
    if (score >= 40) return 'medium';
    return 'low';
  }

  private riskLabel(score: number) {
    const risk = this.riskLevel(score);
    return risk[0].toUpperCase() + risk.slice(1);
  }

  private confidenceScore(sampleSize: number, signals: Array<number | string>) {
  ...
  private totalPredictions(dashboard: PredictionDashboard) {
    return (
      dashboard.staffing.predictions.length +
