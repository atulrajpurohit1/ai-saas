...
export class CopilotQueryService {
  ...
  private async getMissedShiftsLastWeek(tenantId: string) {
    ...
    const rows = new Map<string, { guardId: string; guardName: string; missedShifts: number; shiftIds: string[] }>();
  ...
  private clientName(client: { name: string; companyName?: string | null } | null) {
