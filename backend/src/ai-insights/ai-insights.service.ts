...
type IncidentTrendAccumulator = {
  count: number;
  riskScore: number;
};

type IncidentRiskAccumulator = {
  entityId: string;
  entityType: IncidentRiskRow['entityType'];
  name: string;
...
export class AiInsightsService {
  ...
  async getDashboard(tenantId: string): Promise<AiInsightsDashboard> {
    ...
    const ruleRecommendations = this.buildRuleRecommendations(
      clients,
      guards,
      sites,
      billing,
  ...
  async getClientInsights(tenantId: string): Promise<ClientInsightsResponse> {
    const now = new Date();
    ...
    const incidentByClient = new Map<string, number>();
    ...
    const totalIncidents = rows.reduce(
      (sum, client) => sum + client.incidentCount,
      0,
    );
    const averageIncidents =
      activeClientCount > 0 ? totalIncidents / activeClientCount : 0;
    const topRevenueClient = rows.find((client) => client.revenue > 0);
    const highIncidentClient = rows.find(
      (client) =>
        client.incidentCount > 0 && client.incidentCount > averageIncidents,
    );
    const activeContracts = rows.reduce(
      (sum, client) => sum + client.contractActivity,
      0,
    );
  ...
  async getGuardInsights(tenantId: string): Promise<GuardInsightsResponse> {
    const now = new Date();
    const monthStart = this.startOfMonth(now);
    ...
    const rowsByGuard = new Map<string, GuardInsightRow>();
    ...
    const pastScheduledByGuard = new Map<string, number>();
    ...
    shifts
      ...
      .forEach((shift) => {
        ...
        shift.assignments.forEach((assignment) => {
          ...
          const checkIn = shift.attendanceEvents.find(
            (event) =>
          ...
          if (isPast) {
            pastScheduledByGuard.set(
              assignment.guardId,
              (pastScheduledByGuard.get(assignment.guardId) || 0) + 1,
            );
            if (!checkIn) {
              row.missedShifts += 1;
            }
        ...
        shift.incidents.forEach((incident) => {
          if (!this.isCurrentPeriod(incident.occurredAt, now)) return;
          const row = rowsByGuard.get(incident.guardId);
          if (row) row.incidentCount += 1;
        });
      });
    ...
    const rows = Array.from(rowsByGuard.values())
      ...
      .sort(
        (a, b) =>
          b.missedShifts - a.missedShifts ||
          b.lateCheckIns - a.lateCheckIns ||
          b.attendanceRecords - a.attendanceRecords ||
          a.name.localeCompare(b.name),
      );

    const totalPastScheduled = Array.from(pastScheduledByGuard.values()).reduce(
      (sum, value) => sum + value,
      0,
    );
  ...
  async getSiteInsights(tenantId: string): Promise<SiteInsightsResponse> {
    const now = new Date();
    const monthStart = this.startOfMonth(now);
    ...
    const totalShortageSlots = rows.reduce(
      (sum, site) => sum + site.shortageSlots,
      0,
    );
    const topIncidentSite = rows.find((site) => site.incidentCount > 0);
    const shortageSite = rows.find((site) => site.coverageIssues >= 2);
    const lowAttendanceSite = rows.find(
      (site) => site.attendanceRate !== null && site.attendanceRate < 80,
    );
  ...
  async getBillingInsights(tenantId: string): Promise<BillingInsightsResponse> {
    const now = new Date();
    ...
    const rowsByClient = new Map<string, BillingClientRow>();
    invoices.forEach((invoice) => {
      ...
      if (OUTSTANDING_STATUSES.includes(invoice.status)) {
        row.outstandingAmount = this.roundCurrency(
          row.outstandingAmount + invoice.totalAmount,
        );
      }

      if (this.isDisputedInvoice(invoice)) {
        row.disputedAmount = this.roundCurrency(
          row.disputedAmount + invoice.totalAmount,
        );
      }
    });

    const rows = Array.from(rowsByClient.values()).sort(
      (a, b) =>
    ...
    const lifecycleInvoices = invoices.filter((invoice) =>
      REVENUE_STATUSES.includes(invoice.status),
    );
    const outstandingInvoices = invoices.filter((invoice) =>
      OUTSTANDING_STATUSES.includes(invoice.status),
    );
    const disputedInvoices = invoices.filter((invoice) =>
      this.isDisputedInvoice(invoice),
    );
  ...
  async getIncidentInsights(tenantId: string): Promise<IncidentInsightsResponse> {
    ...
    const dayCounts = new Map<string, IncidentTrendAccumulator>();
    const timeCounts = new Map<string, IncidentTrendAccumulator>();
    const siteRisks = new Map<string, IncidentRiskAccumulator>();
    const clientRisks = new Map<string, IncidentRiskAccumulator>();
    const guardRisks = new Map<string, IncidentRiskAccumulator>();
    ...
    const recurringIncidentTypes = this.buildIncidentTrendRows(
      typeCounts,
      'incident_type',
      'Incident type',
    )
  ...
  private incrementTrend(
    trends: Map<string, IncidentTrendAccumulator>,
    label: string,
    score: number,
  ) {
  ...
  private buildIncidentRecommendations(input: {
    highRiskSites: IncidentRiskRow[];
    clientRisks: IncidentRiskRow[];
    guardRisks: IncidentRiskRow[];
    recurringIncidentTypes: IncidentTrendRow[];
  ...
  private fallbackIncidentSummary(input: {
    totalIncidents: number;
    highRiskSites: IncidentRiskRow[];
    recurringIncidentTypes: IncidentTrendRow[];
    timePatterns: IncidentTrendRow[];
  ...
  private incidentDayLabel(date: Date) {
  ...
  private incidentTimeBucket(date: Date) {
  ...
  private incidentSeverityScore(severity: string) {
  ...
  private riskLevel(score: number): IncidentRiskRow['riskLevel'] {
  ...
  private buildOverview(
    ...
    return {
      summary: [
        this.metric(
          'Active clients',
          this.metricValue(clients, 'Active clients'),
          'Current tenant',
          'info',
        ),
        this.metric(
          'Guard attendance',
          this.metricValue(guards, 'Attendance rate'),
          'This month',
          'info',
        ),
        this.metric(
  ...
  private emptyInsight(
    category: AiInsightItem['category'],
    title: string,
  ): AiInsightItem {
    return {
      id: `${category}-empty`,
  ...
  private metricValue(section: { summary: AiInsightMetric[] }, label: string) {
  ...
  private isDisputedInvoice(invoice: {
    status: string;
