// READ-ONLY audit. No writes, no schema changes, no data changes.
...
async function main() {
  ...
  const migrationRows = await prisma.$queryRawUnsafe<any[]>(
    `SELECT migration_name, checksum, finished_at, applied_steps_count, logs
  ...
  const checkpointCols = await prisma.$queryRawUnsafe<any[]>(
  ...
  const patrolEventCols = await prisma.$queryRawUnsafe<any[]>(
    `SELECT column_name, data_type, is_nullable
     FROM information_schema.columns
