import api from './api';
import { AiInsightMetric, AiRecommendation } from './ai-insights';

export type PredictionRiskLevel = 'low' | 'medium' | 'high';
export type PredictionCategory = 'staffing' | 'incidents' | 'churn' | 'payment' | 'renewal';
...
export async function getPredictionDashboard() {
  const response = await api.get<PredictionDashboard>('ai-predictions');
