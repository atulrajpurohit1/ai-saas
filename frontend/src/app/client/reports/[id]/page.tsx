...
import { DailyServiceReport, getClientReport, isDailyReportSummary } from '@/lib/reports';
import {
  AlertTriangle,
  ArrowLeft,
  CalendarDays,
...
export default function ClientReportDetailPage() {
  ...
  useEffect(() => {
    const fetchReport = async () => {
      ...
      setLoading(true);
      try {
        const data = await getClientReport(reportId);
        setReport(data);
        setError('');
      } catch (err) {
  ...
  const handleDownload = async () => {
    ...
    try {
      const response = await api.get(`client/reports/${report.id}/download`, { responseType: 'blob' });
  ...
  return (
    <ClientLayout>
      <div className="mb-6">
        <Link href="/client/reports" className="inline-flex items-center gap-2 text-sm font-semibold text-slate-400 transition hover:text-white">
          <ArrowLeft size={16} />
          Back to reports
        </Link>
      </div>
          ...
          <section className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-5 sm:p-8">
            ...
            <div className="flex flex-col gap-5 md:flex-row md:items-start md:justify-between">
              <div>
                ...
                <p className="mt-3 text-slate-400">{report.site?.name || 'Linked site'}</p>
              </div>
          ...
          {structuredSummary && (
            <>
              <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                ...
                <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-5">
                  <div className="text-xs font-bold uppercase tracking-widest text-slate-500">Incidents</div>
